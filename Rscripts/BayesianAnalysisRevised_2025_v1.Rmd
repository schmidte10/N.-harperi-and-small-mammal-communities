...
abund_2 <-  abund_1 |> 
  ...
  select(-c(`TRAP MALFUNCTION`, `TRAP DISTURBANCE`)) |>
...
summarise_all(temp1[4:14],funs(sum))/sum(temp1[14])
...
```{r}
ggpredict(comm.comp1.2025, c("SPECIES","TRAPLINE"))
```
...
```{r}
ggpredict(comm.comp2.2025, c("SPECIES","TRAPLINE"))
```
...
perc_rbv.figure <- orangemitedata_v3 |>
  ...
  xlab("% red backed vole in community composition")+ylab("P(Infection)")+ 
...
pdf(file = "outputs/perc_rbv_pop_model_2025.pdf", height = 10, width=14)
perc_rbv.figure
dev.off()
```
...
```{r}
plot(body_size_rbv ~ avg_weight_sampling_period, 
     data =rbv)
```
